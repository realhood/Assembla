...
const addCustomPrompt = () => {
  if (customPrompt.value.trim()) {
    store.addCustomPrompt(customPrompt.value.trim());
    customPrompt.value = '';
...
const generateAnalysis = async () => {
  if (store.selectedAnalysis.length === 0 && !store.customPrompts.length) {
    $toast.error('Please select at least one analysis type or add a custom prompt');
    return;
  }
...
<template>
  <div v-if="isTranscriptionReady" class="max-w-4xl mx-auto space-y-8">
    ...
    <!-- Custom Prompt Modal -->
    <div v-if="showCustomPromptModal" class="fixed inset-0 z-50 overflow-y-auto">
      <div class="fixed inset-0 bg-black bg-opacity-75" @click="showCustomPromptModal = false"></div>
      <div class="flex items-center justify-center min-h-screen p-4">
        <div class="relative bg-gray-900 rounded-xl p-6 max-w-lg w-full border border-purple-500/20">
          <h3 class="text-xl font-bold mb-4 text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-400">
            Add Custom Analysis Prompt
          </h3>
          <textarea
            v-model="customPrompt"
            rows="4"
            class="w-full bg-gray-800 border border-gray-700 rounded-lg p-3 text-white placeholder-gray-500 focus:border-purple-500 focus:ring focus:ring-purple-500/20"
            placeholder="Enter your custom analysis prompt..."
          ></textarea>
          <div class="flex justify-end space-x-3 mt-4">
            <button
              @click="showCustomPromptModal = false"
            ...
            <button
              ...
              class="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 rounded-lg hover:from-purple-700 hover:to-indigo-700"
            >
              Add Prompt
            </button>
          </div>
      ...
      <div class="bg-gray-900/50 backdrop-blur-lg p-8 rounded-2xl shadow-2xl border border-purple-500/20">
        <div class="flex justify-between items-center mb-6">
          ...
          <div class="flex space-x-3">
            <button
              ...
              class="px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded-lg transition-all duration-300"
            >
              Add Custom Prompt
            </button>
            <button
        ...
        <div v-if="showAnalysisSelector" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- Custom Prompts -->
            <div
              v-for="(prompt, index) in store.customPrompts"
              :key="'custom-' + index"
              class="p-4 rounded-xl border bg-purple-900/30 border-purple-500/50"
            >
              <div class="flex justify-between items-start">
                <h3 class="text-lg font-semibold text-indigo-300 mb-2">Custom Analysis {{ index + 1 }}</h3>
                <button
                  @click="store.removeCustomPrompt(index)"
